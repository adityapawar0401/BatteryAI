...
export function parseSuggestions(value: unknown): Suggestions {
  ...
  if (!summary || summary.length > 1000 || /[<>]/.test(summary) || !actions.every((item) => typeof item === "string" && !!item && item.length <= 500 && !/[<>]/.test(item)) || !cautions.every((item) => typeof item === "string" && !!item && item.length <= 500 && !/[<>]/.test(item))) throw new Error("Suggestion output did not match the safe display schema.");
