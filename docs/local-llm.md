...
The fixed system prompt treats summary values as data, prohibits numerical overrides, invented RUL/modalities/history, safety certification and guaranteed maintenance claims, and requests strict JSON only. Ollama receives the exact JSON Schema in `/api/chat`; BatteryAI validates the returned content again with Pydantic. Generated HTML, extra keys, unbounded strings and numerical override fields are rejected. Suggestions cannot modify the authoritative numerical result.
