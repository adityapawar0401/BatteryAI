...
async def validation_exception_handler(_request: Request, exc: ValidationError):
    return JSONResponse(status_code=422, content=ErrorDetail(code="validation_error", message="Input does not satisfy the Oxford V1 contract.", details=jsonable_encoder(exc.errors())).model_dump())
...
def startup_banner(host: str, port: int) -> str:
    return f"BatteryAI local engine\nEndpoint: http://{host}:{port}\nPairing token: {PAIRING_TOKEN}\nDevice request: {os.environ.get('BATTERYAI_DEVICE', 'auto')}"
